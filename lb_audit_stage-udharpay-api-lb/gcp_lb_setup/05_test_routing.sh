...
test_route() {
    ...
    # Check if the expected hostname (from Traefik Whoami) is in the response
    if echo "$RESPONSE" | grep -q "Hostname: ${EXPECTED_HOSTNAME}"; then
...
# P250: rule-engine
test_route "/rule/" "rule-engine" "$STAGING_SUBDOMAIN"

# P280: kyc
test_route "/kyc/" "kyc" "$STAGING_SUBDOMAIN"
