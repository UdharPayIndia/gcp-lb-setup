#!/bin/bash
# ==============================================================================
# 01_vms.sh — Create a single GCP Compute Engine VM for all staging services
# ==============================================================================
# One general-purpose VM hosts ALL staging services on different ports.
# In AWS, these were spread across 6 EC2 instances — here we consolidate.
#
# Services & Ports on this VM:
#   staging-support-service        → 8089
#   stage-users-and-verifications  → 8081
#   staging-account-preference     → 8086
#   stage-payments-and-billing     → 8082
#   staging-udharpay-api           → 8000
#   staging-superkey               → 8083
#   stage-dlt                      → 8082  (shares port with payments, adjust if needed)
#   staging-platform-rule-engine   → 8081  (shares port with users, adjust if needed)
...
#
# ⚠️  NOTE: Several services share ports in the original AWS setup because
# they ran on DIFFERENT VMs. On a single VM, you MUST assign unique ports
# to each service. Update the port mappings in 02_target_groups.sh accordingly.
# ==============================================================================
...
echo ""
echo "NOTE: Deploy all 19 staging services (Docker containers) on this VM,"
