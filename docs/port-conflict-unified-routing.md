...
| :--- | :---: | :---: | :---: | :--- | :--- |
| `udharpay-api` | 8000 | 8000 | ✅ OK (Cloud Run) | `/v2/merchant/*`, `/api/*`, etc. | Y |
| `users-verifications` | 8081 | 8081 | ✅ OK (Cloud Run) | `/verification/*`, `/auth/*`, etc. | Y |
| `payments-billing` | 8082 | 8082 | ✅ OK (Cloud Run) | `/payments-and-billing/*`, etc. | Y |
| `superkey` | 8083 | 8083 | ✅ OK (Cloud Run) | `/superkey/*`, `/rocketscore/*`, etc. | Y |
...
| `workflow` | 8246 | 8246 | ✅ OK (Cloud Run) | `/workflow/*` | Y |
...
### From: `staging-udharpay-admin-lb` (AWS) / `load-balancers/stage-udharpay-admin/` (repo)
...
*Note: The 5 core backend services migrated to Cloud Run (`users-verifications`, `payments-billing`, `udharpay-api`, `superkey`, and `workflow`) are inherently isolated from host-level port conflicts as they have dedicated routing from Serverless NEGs.*
