...
echo ""
echo "=== 6.5/11: Serverless NEGs ==="
SERVERLESS_NEGS=(
  stage-neg-as1-users-verifications
  stage-neg-as1-payments-billing
  stage-neg-as1-udharpay-api
  stage-neg-as1-superkey
  stage-neg-as1-workflow
)
for neg in "${SERVERLESS_NEGS[@]}"; do
  del gcloud compute network-endpoint-groups delete "$neg" --region="$REGION"
done
