...
| **Target Group equivalent** | Target Group (TG) | Backend Service (BS) with Instance Group or Serverless NEG |
| **Health Check** | Defined on Target Group | Global Health Check Resource (N/A for Serverless NEGs) |
...
- **GCP**: 
  - Using 1 consolidated VM for legacy services. We had to shift conflicting services to new ports (e.g., `rule-engine` moved to `8084`) while keeping the Load Balancer "Named Ports" mapped to these new values.
  - **Cloud Run (NEW)**: 5 core microservices (`users-verifications`, `payments-billing`, `udharpay-api`, `superkey`, `workflow`) were fully migrated off the VM into Serverless Cloud Run Containers, utilizing Serverless NEGs to attach to the Load Balancer natively. This avoids any port conflicts entirely for these 5 services.
